...
| :-: | :- | :- | :- |
| 1 | сотрудник колцентра, менеджер отделения, клиент | Регистрация нового клиента | 1) клиент регистрируется на сайте 2) подтверждает номер телефона через смс 3) заявка добавляется в АБС 4) сотрудник колцентра звонит новому клиенту и назначает дату и время для подтверждение личности в офисе 5) менеджер отделения подтверждает личность |
| 2 | сотрудник колцентра, менеджер отделения, клиент | Оформление депозита | 1) выбирает депозит на сайте/интернет-банке 2) клиент вводит номер счёта и сумму, и отправляет заявку 3) подтверждает личность через смс код 4) заявка добавляется в АБС 5) сотрудник колцентра звонит клиенту и назначает дату и время для оформления депозита в офисе 5) клиент получает услугу в отделении |
| 3 | сотрудник колцентра, менеджер отделения, клиент | Оформление кредита | 1) клиент ознакамливается с персональными условиями кредита на сайте/интернет-банке 2) если условия устраивают отправляет заявку 3) подтверждает личность через смс код 4) заявка добавляется в АБС 5) сотрудник колцентра звонит клиенту и назначает дату и время для оформления кредита в офисе 5) клиент получает услугу в отделении |
|||||
...
#### Context
<img src="../out/Task3/context/StandartBankContext.png" height="400px" />

#### Container
<img src="../out/Task3/container/StandartBankContainer.png" height="400px" />
